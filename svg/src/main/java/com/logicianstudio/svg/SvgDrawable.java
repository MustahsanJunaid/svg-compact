...
public class SvgDrawable extends PictureDrawable {
    ...
    public SvgDrawable(@Nullable Picture picture) {
        super(picture);
    }
     ...
     */
    @TargetApi(Build.VERSION_CODES.HONEYCOMB)
    public SvgDrawable(@Nullable View view, @Nullable Picture picture) {
        super(picture);
        prepareView(view);
    }

    /**
     * Prepare a view for rendering this SvgDrawable by setting its LayerType to
     * LAYER_TYPE_SOFTWARE.
     *
     * @param view
     */
    ...
    public void draw(Canvas parentCanvas) {
        ...
        if (SvgCompact.LOG_LEVEL >= SvgCompact.LOG_LEVEL_INFO) {
     ...
     * automatically redrawn, as the way in which the SvgDrawable is drawn may change without it
